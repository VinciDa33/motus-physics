...
public abstract class Collider
{
    ...
    public static CircleCollider CreateCircleCollider(double radius)
    {
        if (radius <= 0d)
        {
            Motus.Logger.LogWarning("Circle collider radius must be positive!\nRadius has been set to 0.01.");
            radius = 0.01d;
        }
    ...
    public static PolygonCollider CreateRectangleCollider(Vector size)
    {
        if (size.x <= 0d)
        {
            Motus.Logger.LogWarning("Rectangle collider width must be positive!\nWidth has been set to 0.01.");
            size.x = 0.01d;
        }
        
        if (size.y <= 0d)
        {
            Motus.Logger.LogWarning("Rectangle collider height must be positive!\nHeight has been set to 0.01.");
            size.y = 0.01d;
        }
    ...
    public static PolygonCollider CreatePolygonCollider(params Vector[] points)
    {
        if (points.Length < 3)
        {
            Motus.Logger.LogError("Polygon collider must have 3 or more vertices!\nA default rectangle shape has been created instead!");
